...
class _MyQrState extends State<MyQr> {
  ...
  Future<void> _mintHandoffToken() async {
    try {
      ...
      if (mounted) {
        setState(() => _handoffError = 'Cannot reach SCWT right now.');
      }
    }
  }
  ...
  String _nextQrData() {
    ...
    return 'SCWT:${AppStore.instance.user?.studentId ?? 'ANONYMOUS'}:${DateTime.now().millisecondsSinceEpoch}';
  }
  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ScreenHeader(title: 'My SCWT QR', back: true, go: widget.go),
              ...
              children: [
                Text(
                  inSession
                      ...
                      : 'Scan this code at an SCWT station',
                  style: const TextStyle(
                    fontSize: 13,
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 4),
          const SectionHeading('Nearby SCWT Stations'),
          Column(
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            child: Text(
              AppStore.instance.currentStation?.name ?? 'SCWT Station',
              style: TextStyle(
                color: Colors.white.withValues(alpha: 0.9),
                fontSize: 12,
                fontWeight: FontWeight.w700,
...
class ContributionDetail extends StatelessWidget {
  ...
  String _stationName(String id) {
    if (id.isEmpty) return 'SCWT Station';
    final match = AppRepositories.instance.stationsSync().where(
      (s) => s.id == id,
    );
    return match.isEmpty ? 'SCWT Station' : match.first.name;
  }
}
  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
          ScreenHeader(title: 'SCWT Faculty Cup', back: true, go: widget.go),
          const SizedBox(height: 10),
          Container(
            padding: const EdgeInsets.all(14),
            decoration: BoxDecoration(
